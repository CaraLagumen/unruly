...
export const validateDelete = catchAsync(async (req, res, next) => {
  ...
  if (moment(scheduled?.date) < schedulingWeek) {
    return next(
      new AppError(
...
export const createScheduled = catchAsync(async (req, res, next) => {
  ...
  const doc = await Scheduled.create<IScheduledData>({
    shift,
    employee,
    scheduler,
    date,
...
export const deleteLastScheduled = catchAsync(async (req, res, next) => {
  ...
  if (moment(lastScheduled?.date) < schedulingWeek) {
    return next(
      new AppError(
        `Last scheduled is in the past or is on this coming week. Cannot delete.`,
        400
      )
    );
  }
